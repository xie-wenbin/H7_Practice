...
{ 
    ...
    // inputColor = CONVERTRGB5652ARGB8888(*pencolor);
	uint16_t width=rect->right-rect->left+1;		//填充的宽度
	uint16_t height=rect->bottom-rect->top+1;	//填充的高度 
	gPic_Drv.fill_color(rect->left+gPicInfo.set_xoff,rect->top+gPicInfo.set_yoff,width,height,(uint32_t*)pencolor);//颜色填充 
...
{ 
	...
	{
		realy=(gPicInfo.div_fac*(rect->top+i))>>13;//实际Y坐标											 
		//在这里不改变picinfo.staticx和picinfo.staticy的值 ,如果在这里改变,则会造成每块的第一个点不显示!!!
		if(!Pic_isElementValid(realx,realy,0))//行值是否满足条件? 寻找满足条件的行
		{ 
			pencolor+=width;
			continue;
		}
		...
		{
			realx=(gPicInfo.div_fac*(rect->left+j))>>13;//实际X坐标
			//在这里改变picinfo.staticx和picinfo.staticy的值
			if(!Pic_isElementValid(realx,realy,1))//列值是否满足条件? 寻找满足条件的列
			{
				pencolor++;
				continue;
			}
			gPic_Drv.draw_point(realx+gPicInfo.set_xoff,realy+gPicInfo.set_yoff,*pencolor);//显示图片						
			pencolor++;
		}
	}
    return 0;    //返回0,使得解码工作继续执行 
	...
	if(res==0)
	{
		...
		{ 
			...
			{ 	
				...
				{ 
					if((jpeg_dev->width>>scale)<=gPicInfo.set_width&&(jpeg_dev->height>>scale)<=gPicInfo.set_height)//在目标区域内
					{	
						if(((jpeg_dev->width>>scale)!=gPicInfo.set_width)&&((jpeg_dev->height>>scale)!=gPicInfo.set_height&&scale))scale=0;//不能贴边,则不缩放
				...
				gPicInfo.imgheight=jpeg_dev->height>>scale;	//缩放后的图片尺寸
				gPicInfo.imgwidth=jpeg_dev->width>>scale;	//缩放后的图片尺寸 
				Pic_drawInit();								//初始化智能画图 
