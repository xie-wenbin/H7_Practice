...
int32_t BSP_I2C4_WriteReg(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
    ...
    if (HAL_I2C_Mem_Write(&hbus_i2c4, DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length, 1000) == HAL_OK)
    {
        ret = BSP_ERROR_NONE;
    }
    else
...
int32_t BSP_I2C4_ReadReg(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
    ...
    osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif
    if (HAL_I2C_Mem_Read(&hbus_i2c4, DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length, 1000) == HAL_OK)
    {
        ret = BSP_ERROR_NONE;
    }
    else
